...
const Popup: React.FC = () => {
    ...
    const loadUserInfo = async () => {
      try {
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            if (!chrome.runtime.lastError && response && response.environment) {
  ...
  const handleOpenWebsite = () => {
    chrome.tabs.create({ url: 'https://avaliadorprompt.com' });
  };
  
  const handleOpenSettings = () => {
    chrome.tabs.create({ url: 'https://avaliadorprompt.com/account' });
  };

  const toggleEnvironment = () => {
    const newEnvironment: Environment = environment === 'production' ? 'development' : 'production';
    ...
    <div className="popup-container">
      ...
      <div className="popup-content">
        ...
        <div className="environment-selector">
          ...
          <button 
            className={`environment-button ${environment === 'production' ? 'active' : ''}`}
            onClick={toggleEnvironment}
          >
            {environment === 'production' ? 'Produção' : 'Desenvolvimento'}
          </button>
          <span className="env-indicator">{environment === 'production' ? '🌐' : '💻'}</span>
        </div>
